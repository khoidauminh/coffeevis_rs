...
//use crate::assets::coffee_pixart_map::cp;
...
pub unsafe fn prepare_img(file: &[u8]) {
    //let path = Path::new(path_string);

//    let mut f = File::open(
//        path_string
//   ).expect(&format!("Failed to open file: {:?}", path_string));
...
pub unsafe fn draw_shaky(buf : &mut [u32], stream : &[(f32, f32)]) {
