...
impl ApplicationHandler for WindowState {
    ...
    fn window_event(
        ...
        match event {
            ...
            WindowEvent::KeyboardInput { event, .. } => match self.commands.try_write() {
...
pub fn winit_main(mut prog: Program) -> Result<(), &'static str> {
    ...
    let thread_main_running_ = state.thread_main_running.clone();

    let thread = thread::spawn(move || {
        let mut inner_size = inner_size;
        ...
        while thread_main_running.load(Relaxed) {
            ...
            if draw {
                if (size.width != inner_size.width || size.height != inner_size.height) {
