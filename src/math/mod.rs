...
fn transpose(a: &mut [(f32, f32)], c: usize) {
    let l = a.len();
    let ll = l-1;
    let lh = l/2;
...
pub fn fft_inplace(a: &mut Vec<(f32, f32)>) {
    let l = a.len();
...
pub fn euler_wrap(n : (f32, f32), a : f32) -> (f32, f32) {
...
pub fn complex_mag(a: (f32, f32)) -> f32 {
    fast_fsqrt(a.0.powi(2) + a.1.powi(2))
}

// ------------------------------------------------------------------------- //

pub fn dft(inp: &[(f32, f32)], out: &mut [(f32, f32)], freq_bound: f32) {
    let l = inp.len();
    let lb = out.len();
...
pub fn log2i(mut n : usize) -> usize {
	(size_of_usize - n.leading_zeros() as usize -1) as usize
} 
